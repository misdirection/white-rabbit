...
export function setupGUI(planets, sun, orbitGroup, zodiacGroup) {
    ...
    const speedSlider = controlsFolder.add(uiState, 'speedExponent', 0, 6).name('Speed (Log)').onChange(val => {
        if (val <= 0.1) {
    ...
    const planetSlider = controlsFolder.add(config, 'planetScale', 0.1, 5).name('Planet Scale').onChange(val => {
    ...
    const sunSlider = controlsFolder.add(config, 'sunScale', 0.1, 5).name('Sun Scale').onChange(val => {
